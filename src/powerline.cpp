...
void PowerlineDetector::detectPowerLines(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
{
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr power_line_with_distances(new pcl::PointCloud<pcl::PointXYZI>);
...
void PowerlineDetector::updateImage(const pcl::PointXYZ &point, float distance)
{
    // Convert map coordinates to image pixel coordinates
    int img_x = static_cast<int>((point.x - origin_x_) / meters_per_pixel_);// + image_size_.width / 2;
    int img_y = static_cast<int>((point.y - origin_y_) / meters_per_pixel_);// + image_size_.height / 2;
    ...
    if (img_x >= 0 && img_x < distance_matrix_.cols && img_y >= 0 && img_y < distance_matrix_.rows)
    {
...
void PowerlineDetector::saveGeoTIFF()
{
    // Convert `origin_x_` and `origin_y_` to UTM
    double utm_x, utm_y;
    try
    {
        geometry_msgs::msg::TransformStamped transform_stamped = tf_buffer_.lookupTransform("utm", "map", tf2::TimePointZero, tf2::durationFromSec(1.0));

        utm_x = transform_stamped.transform.translation.x + origin_x_;
        utm_y = transform_stamped.transform.translation.y - origin_y_;

        RCLCPP_INFO(this->get_logger(), "✅ Transformed origin (%.2f, %.2f) → UTM (%.2f, %.2f)", origin_x_, origin_y_, utm_x, utm_y);
    }
    ...
    for (int i = 0; i < 3; i++)
    {
        CPLErr err = dataset->GetRasterBand(i + 1)->RasterIO(GF_Write, 0, 0, color_image.cols, color_image.rows,
                                                              channels[i].data, color_image.cols, color_image.rows,
                                                              GDT_Byte, 0, 0);
        if (err != CE_None)
        {
    ...
    GDALClose(dataset);
    RCLCPP_INFO(this->get_logger(), "✅ Successfully saved powerline distances to: %s", filename.c_str());
}