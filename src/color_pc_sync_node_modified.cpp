...
// 	PointXYZRGBIL,
// 	(float, x, x)(float, y, y)(float, z, z)(float, rgb, rgb)(uint32_t, label, label)(float,
// intensity, intensity))
...
class ImageLivoxFusion {
  ...
  public:
    ImageLivoxFusion() : it(nh) {
        ...
        image_sub =
            it.subscribe("/mapir_rgn/image_raw", 100, &ImageLivoxFusion::imageCallback, this);
        // livox_sub = nh.subscribe("/livox/lidar", 100, &ImageLivoxFusion::livoxCallback, this);

        // TODO replace sub raw/pub rectified with synced to raw and rectify it in the synced
        // callback
        ...
        image_sync_sub = new message_filters::Subscriber<sensor_msgs::Image>(
            nh, "/hik_cam_node/undist_camera", 1);
        livox_sync_sub =
            new message_filters::Subscriber<sensor_msgs::PointCloud2>(nh, "livox/lidar", 1);
        // multi sensor synchronized
        sync_ = new message_filters::Synchronizer<syncPolicy>(syncPolicy(10), *image_sync_sub,
                                                              *livox_sync_sub);
    ...
    void integral_callback(const sensor_msgs::ImageConstPtr &img_msg,
...
void ImageLivoxFusion::set_param() {
    ...
    if (!nh.getParam("/color_pc_sync_modified_node/CameraExtrinsicMat/data", param_list))
    ...
    if (!nh.getParam("/color_pc_sync_modified_node/CameraMat/data", param_list))
        ROS_ERROR("Failed to get extrinsic parameter.");
    for (size_t i = 0; i < param_list.size(); ++i) {
        XmlRpc::XmlRpcValue tmp_value = param_list[i];
        if (tmp_value.getType() == XmlRpc::XmlRpcValue::TypeDouble) {
    ...
    if (!nh.getParam("/color_pc_sync_modified_node/DistCoeff/data", param_list))
        ROS_ERROR("Failed to get extrinsic parameter.");
    for (size_t i = 0; i < param_list.size(); ++i) {
        XmlRpc::XmlRpcValue tmp_value = param_list[i];
        if (tmp_value.getType() == XmlRpc::XmlRpcValue::TypeDouble) {
    ...
    if (!nh.getParam("/color_pc_sync_modified_node/ImageSize", param_list))
        ROS_ERROR("Failed to get extrinsic parameter.");
    for (size_t i = 0; i < param_list.size(); ++i) {
        XmlRpc::XmlRpcValue tmp_value = param_list[i];
...
void ImageLivoxFusion::imageCallback(const sensor_msgs::ImageConstPtr &msg) {
    ...
    cv::resize(cv_ptr->image, resized_image, cv::Size(1920, 1080));
    ...
    // linshi_msg = cv_bridge::CvImage(std_msgs::Header(), sensor_msgs::image_encodings::BGR8,
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (pointRGB.x >= 0) {
            if (x >= 0 && x < img_width && y >= 0 && y < img_height) {
                ...
                // pointRGB.intensity = linshi_raw_pcl_ptr->points[i].intensity;
                // //继承之前点云的intensity
                pc_xyzrgb->push_back(pointRGB);
            }
        }
    }
    // visualizePointCloud(pc_xyzrgb);
    pc_xyzrgb->width = 1;
...
int main(int argc, char **argv) {
    ...
    if (argc != 2) {
        ROS_ERROR("need is_correct as argument");
        return -1;
    };

    if (strcmp(argv[1], "true") == 0) {
        IS_IMAGE_CORRECTION = true;
        ROS_INFO("correct image");
    } else {
        IS_IMAGE_CORRECTION = false;
        ROS_INFO("don't correct image");
    }
    ImageLivoxFusion ic;
