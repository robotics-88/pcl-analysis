    ...
    , has_first_trailpt_(false)
{
    ...
    mavros_local_pos_subscriber_ = this->create_subscription<geometry_msgs::msg::PoseStamped>("/mavros/vision_pose/pose", rclcpp::SensorDataQoS(), std::bind(&Trail::localPositionCallback, this, _1));

    point_cloud_subscriber_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(point_cloud_topic_, 10, std::bind(&Trail::pointCloudCallback, this, _1));
    ...
    trail_enabled_service_ = this->create_service<rcl_interfaces::srv::SetParametersAtomically>("trail_enabled_service", std::bind(&Trail::setTrailsEnabled, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void Trail::findAngle(const double x1, const double y1, const double theta1, const double x2, const double y2, double &theta_out)
{
...
pcl::PointCloud<pcl::PointXYZ>::Ptr Trail::findMaximumPlanar(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_clustered,
...
bool Trail::setTrailsEnabled(const std::shared_ptr<rmw_request_id_t>/*request_header*/,
